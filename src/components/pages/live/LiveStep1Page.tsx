...
const COURSE_ID = 'ai-building-course';
const DAY_OF_WEEK = 2; // 화요일
...
interface LiveStep1PageProps {
  onBack?: () => void;
}

const LiveStep1Page: React.FC<LiveStep1PageProps> = ({ onBack }) => {
    ...
    const initialize = async () => {
      ...
      if (userSession) {
        try {
          ...
          const hasAccess = enrollments.some((e: { courseId: string }) => 
            e.courseId === 'ai-building-course' || e.courseId === 'ai-building' || e.courseId === '999'
          );
  ...
  if (!isEnrolled) {
    return (
      <div style={{ minHeight: '100vh', background: COLORS.navy }}>
        <NavigationBar onBack={onBack || (() => navigate('/ai-building-course/player'))} breadcrumbText="AI 건물주 라이브" />
  ...
  const isLiveNow = liveConfig?.isLive && liveConfig?.liveUrl;
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: 'clamp(20px, 4vw, 40px) clamp(15px, 3vw, 20px)' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          <div style={{ 
            ...
            <span style={{ color: COLORS.white, fontWeight: '700', fontSize: '0.85rem' }}>STEP 1 · 수강생 전용</span>
          </div>
        ...
        {isLiveNow ? (
          ...
          <div style={{
            ...
            <div style={{ color: COLORS.blueLight, fontSize: '1.3rem', fontWeight: '700', marginBottom: '8px' }}>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: COLORS.grayDark, fontSize: '0.9rem' }}>
                      {archive.date} {archive.duration && `· ${archive.duration}`}
                    </div>
                  </div>
        ...
        <div style={{
          ...
          <h4 style={{ color: COLORS.blueLight, fontWeight: '700', marginBottom: '10px', fontSize: '1rem' }}>
            📌 라이브 안내
          </h4>
          <ul style={{ color: COLORS.gray, fontSize: '0.9rem', lineHeight: '1.8', margin: 0, paddingLeft: '18px' }}>
            <li>매주 화요일 오후 8시에 라이브가 진행됩니다</li>
